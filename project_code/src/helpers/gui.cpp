...
bool gui_parameters::display()
{
 ...
 //   new_surface |= ImGui::RadioButton("Mesh",ptr_surface_type, surface_mesh);
	...
	ImGui::SliderFloat("Uplift", &uplift, 0.f, 1.f);
	ImGui::SliderFloat("Radius", &radius, 0.1f, 1.f);

	ImGui::SliderFloat("(In simultation) log of time interval", &log_dt, -0.7f, -0.01f);
	ImGui::SliderFloat("Parameter k", &param_k, 0.01f, 10.f);
